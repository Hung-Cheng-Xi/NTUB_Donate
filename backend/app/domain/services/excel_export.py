...
class ExcelService:
            ...
            transformed_data.append(
                {
                    "編號": data.id,
                    "受捐單位": (
                        data.purpose.unit.name if data.purpose.unit else "N/A"
                    ),
    ...
    async def create_workbook(self, skip: int, limit: int):
        ...
        # 將資料轉換為字典列表
        export_data = [
            ExcelExportInfo.model_validate(record) for record in records
        ]

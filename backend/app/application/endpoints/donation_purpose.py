...
async def read_donation_purposes(db: AsyncSession = Depends(get_db_session)):
...
async def create_donation_purpose(new_donation_purpose: DonationPurposeCreate, db: AsyncSession = Depends(get_db_session)):
    donation_purpose_repo = DonationPurposeRepository(db)
